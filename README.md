...
- **Article tagging (optional)** — set `EXPO_PUBLIC_USE_ARTICLE_TAGS=true` in `.env` to honour the server's article-tagging metadata (`geo_tags` / `entities` / `event_type`) when scoring. **Defaults to off**, which treats every article as untagged so all scoring goes down the legacy two-pass LLM path — the correct setting against a backend whose enrichment stage is not running. With it on, tagged articles take the deterministic on-device math path instead. Only the exact string `"true"` enables it; the resulting math-vs-AI split is reported on the in-app Observability screen (Settings → Observability → feed funnel), so the two scorers can be compared side by side on the same feed.
