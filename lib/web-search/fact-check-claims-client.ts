...
import {
  SEARCH_UNAVAILABLE,
  raceDeadline,
  unavailableCodeForStatus,
  type SearchUnavailableCode,
...
export async function searchClaimReviews(
  ...
  // The deadline covers the QUEUE WAIT as well as the request — see the same
  // note in web-search-client. The limiter's FIFO has no ceiling, so timing only
  // the fetch would leave a lookup able to hang indefinitely.
