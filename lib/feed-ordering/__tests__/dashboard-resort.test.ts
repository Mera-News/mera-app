...
describe('DASHBOARD_RESORT_INTERVAL', () => {
  it('is 10 minutes, in both units', () => {
    expect(DASHBOARD_RESORT_INTERVAL_MINUTES).toBe(10);
    expect(DASHBOARD_RESORT_INTERVAL_MS).toBe(10 * MIN);
  });
});
...
describe('shouldResort', () => {
  ...
  it('refuses before the interval, even when the user looks away', () => {
    expect(
      shouldResort({ lastAppliedMs: T0, nowMs: T0 + 9 * MIN, trigger: 'unwatched' }),
    ).toBe(false);
  });
  ...
  it('allows exactly at the mark', () => {
    expect(
      shouldResort({ lastAppliedMs: T0, nowMs: T0 + 10 * MIN, trigger: 'unwatched' }),
    ).toBe(true);
  });

  // A user who never looks away still gets a converging order.
  it('allows past the interval while still watching', () => {
    expect(shouldResort({ lastAppliedMs: T0, nowMs: T0 + 15 * MIN, trigger: 'elapsed' })).toBe(true);
  });
...
describe('msUntilResortDue', () => {
  ...
  it('counts down to the mark', () => {
    expect(msUntilResortDue(T0, T0 + 4 * MIN)).toBe(6 * MIN);
  });
