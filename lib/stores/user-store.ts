...
export const useUserStore = create<UserState>()((set, get) => ({
    ...
    fetchUserPersona: async (userId, force = false) => {
        // Guard against empty/falsy ids — the server rejects these as
        // "Access denied: resource belongs to another user" and Apollo then
        // treats it as an auth failure. Callers are expected to wait until
        // a real userId is available.
        if (!userId) {
            return null;
        }
