...
export class ArticleFeedbackAgent implements IAgent {
  ...
  async buildSystemPrompt(needsToolFormat: boolean): Promise<string> {
    ...
    this.cachedSystemPrompt = buildArticleFeedbackPrompt({ needsToolFormat, languageName });
    this.lastNeedsToolFormat = needsToolFormat;
    this.lastLanguageName = languageName;
    this.lastMode = mode;
    return this.cachedSystemPrompt;
  ...
  async buildContext(): Promise<string> {
    ...
    } else if (ctx.suggestion.status === ArticleSuggestionStatus.Complete) {
  ...
  async executeTool(name: string, input: unknown): Promise<ToolExecutionResult> {
    ...
    switch (name) {
      case 'proposeChanges':
        return this.handleProposeChanges(args);
  ...
  private async handleProposeChanges(args: Record<string, unknown>): Promise<ToolExecutionResult> {
    ...
    // Validate every action shape + referenced fact ids in a single getFacts pass.
    ...
    // Does NOT touch the DB — only stages the proposal for user confirmation.
    // proposalId is echoed in the result so deriveThreadItems can key the
    // rebuilt proposal card to store.proposal / resolvedProposals.
    return {
...
function buildArticleFeedbackPrompt(params: {
