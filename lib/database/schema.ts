  ...
  tables: [
    ...
    tableSchema({
      ...
      columns: [
        ...
        // ── Scorer VINTAGE (schema v50) ────────────────────────────────
        // Which relevance scorer produced this row's `relevance`, so the
        // render gate can be applied PER ROW instead of per flag. v1 and v3
        // scores are not on the same scale: v1's are quantised (25 distinct
        // values over the 348-row gold set, with 25 rows at EXACTLY 0.4)
        // while v3's are continuous, which is why v3 renders at 0.55 and v1
        // at 0.4. Flipping one global gate re-judges rows that were never
        // scored by the scorer the gate was calibrated for — on that gold set
        // a flag-level 0.55 gate silently deletes the 25 rows sitting at
        // exactly 0.4 (21.6% of everything v1 admits) with no re-score that
    ...
    // Machine translations of server English text (titles, descriptions,
    // reasons) into the user's app language, produced by the OS translator.
    //
    // WHY IT EXISTS. The cache used to be an in-memory `Map` in
    // `lib/stores/app-language-store` with no persistence at all: every
    // translation was lost on app restart AND wiped wholesale on a language
    // switch. A non-English reader therefore paid the full serial native-call
    // cost on EVERY cold start, for text the device had already translated —
    // which is the bulk of what "translation is slow" actually felt like.
    //
    // DERIVED, NOT USER-OWNED: every row can be regenerated by asking the OS
    // again, so this table is safe to sweep (and would be safe to drop). It is
    // the only table here of which that is true — do not read the sweep below
    // as licence to wipe anything else.
    //
    // The row `id` IS `${source_hash}:${target_lang}`, so a lookup is a primary
    // key hit with no index and no query planning. `source_text` is stored in
    // full and re-checked on read, so a 32-bit hash collision can only ever
    // cost a cache MISS, never a wrong translation.
    tableSchema({
