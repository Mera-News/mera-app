            ...
            <Box className="flex-1">
                ...
                <ScrollView className="flex-1 px-5" contentContainerStyle={{ paddingBottom: insets.bottom + 24 }}>
                    ...
                    {/* Backup is the one CONSTRUCTIVE thing on this screen, so it
                        sits above the list and is deliberately not drawn in the
                        destructive red every row below uses. It is opt-in: this
                        row is an entry point, not a switch, and nothing runs
                        until setup finishes on the other side. */}
                    <Pressable
                        testID="manage-data-backup"
                        className="flex-row items-center py-4 px-4 border border-gray-700 rounded-lg mb-5"
                        // Cast, like the five other router.push sites here: the
                        // typed-route union in .expo/types/router.d.ts is
                        // generated by `expo start` and does not know a route
                        // added since the last run.
                        onPress={() => router.push('/logged-in/preferences/backup' as never)}
                        disabled={isProcessing || isDeletingAccount}
                    >
                        <MaterialIcons name="cloud-upload" size={22} color="#ffffff" />
                        <VStack className="ml-3 flex-1">
                            <Box className="flex-row items-center">
                                <Text className="text-base text-white">{t('backup.title')}</Text>
                                {renderLocationChip('device')}
                            </Box>
                            <Text size="xs" className="text-gray-500">{t('backup.entryDescription')}</Text>
                        </VStack>
