 ...
 *
 * THIS REVERSES PIVOT P8h IN THE OPPOSITE DIRECTION FROM ITS OWN FIX. P8h
 * (see `fact-check-state.ts`'s now-deprecated `describeVerdictPresentation`)
 * made an organisation's rating outrank a confident Mera verdict once one
 * existed, but still let Mera's own verdict lead the chip whenever
 * `checkedBy` was empty — and a real screenshot caught exactly that leading
 * to a false headline once: a true story, several off-topic externals rated
 * False, Mera's own well-evidenced "Consistent with sources" still won the
 * chip because it was the EXTERNALS that were actually irrelevant, not
 * absent. This wave removes Mera's verdict from the chip and body ENTIRELY
 * rather than trying to rank it against externals: the two failure modes (a
 * wrong Mera verdict heading a true story, and a right-but-off-topic
 * external heading it) share one root cause — the client has no relevance
 * signal for individual `checkedBy` entries, see `fact-check-types.ts` — so
 * removing Mera's verdict from the decision surface entirely closes both at
 * once instead of re-ranking them.
 */
